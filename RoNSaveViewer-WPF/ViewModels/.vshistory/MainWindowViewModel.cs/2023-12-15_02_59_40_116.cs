            ...
            set
            {
                ...
                if (value != null)
                {
                    switch (value.OBJUProperty.Type)
                    {
                        ...
                        case "MapProperty":
                            KeyValuePair<ObjectProperty, ObjectProperty>[] val2 = (KeyValuePair<ObjectProperty, ObjectProperty>[])value.OBJUProperty.Value;

                            foreach (var item in val2)
                            {
                                EditableRoNSaveObjects.Add(new EditableRoNSaveObject(item.Key));
                                EditableRoNSaveObjects.Add(new EditableRoNSaveObject(item.Value));
                            }
                            break;
