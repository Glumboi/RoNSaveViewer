            ...
            set
            {
                ...
                if (value != null)
                {
                    switch (value.OBJUProperty.Type)
                    {
                        ...
                        case "MapProperty":
                            // Assuming your original object is of type object
                            object originalObject = value.OBJUProperty.Value;

                            // Check if the object is a List<KeyValuePair<UProperty, UProperty>>
                            if (originalObject is List<KeyValuePair<UProperty, UProperty>>)
                            {
                                ...
                                // Now you can work with the originalList

                                // Add UI items based on originalList or perform other operations
                                foreach (var pair in originalList)
                                {
                                    AddUIItem(pair.Key);
                                    AddUIItem(pair.Value);
                                }
                            }
                            else
                            {
