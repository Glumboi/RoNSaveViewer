            ...
            set
            {
                ...
                if (value != null)
                {
                    switch (value.OBJUProperty.Type)
                    {
                        ...
                        case "MapProperty":
                            var mapValue = value.OBJUProperty.Value;

                            if (mapValue != null)
                            {
                                var keyValuePairs = mapValue.GetType().GetProperty("KeyValuePairs")?.GetValue(mapValue) as IEnumerable;

                                if (keyValuePairs != null)
                                {
                                    foreach (var keyValuePair in keyValuePairs as UProperty)
                                    {
                                        // Assuming each keyValuePair is of type UProperty
                                        SaveObjects.Add(keyValuePair);
                                    }
                                }
                            }
                            break;
